...
--
-- This is a slightly modified version of default parsec Token module.
-- The only change is that simpleSpace internal parser now does not consume
-- newlines, so we can use newlines in the grammar built with buildExpressionParser.
